...
author: "Alexander Brenning & Donovan Bangs"
...
Running the geoprocessing functions requires that SAGA is installed and can be found by RSAGA. Multiple versions of SAGA may be accessible, and the environment changed through the `rsaga.env()` function. To check which version of SAGA is used by default with RSAGA, print the environment:

```{r, collapse = TRUE}
rsaga.env()
```

In this case, an older version of SAGA (`$version`) is located on the default path. To change the environment to an updated version of SAGA, point the environment to the directory containing `saga_cmd.exe` of the desired SAGA version, _e.g._:
...
The object `env` may now be used to direct RSAGA functions to use this specific version of SAGA. Version `2.2.0` will be used for this tutorial by specifying `env = env` in functions that use this SAGA environment.
...
![Terrain variables a) slope; b) plan curvature; c) profile curvature; d) logarithm of contributing area](terrain_variables.png)
...
![Landslide probability prediction grid](lslpred.png)
...
## A Note on Versions
As SAGA-GIS is regularly updated, we strive to keep RSAGA functional with as many SAGA versions as practicable. However, the release version of RSAGA on CRAN may not be compatible with all versions of SAGA-GIS. Currently, RSAGA supports SAGA versions 2.0.4 and higher.

When a new version of SAGA-GIS is released, RSAGA on CRAN may not be initially compatible with this release. Development and pre-release versions of RSAGA may be downloaded from the project Github page at: <http://github.com/debangs/RSAGA>
...
Hastie, T. (2015). gam: Generalized Additive Models. R package version 1.12. http://CRAN.R-project.org/package=gam
...
Ungerechts, L. (2010): DEM 10m (triangulated from aerial photo - b/w). Available online (http://www.tropicalmountainforest.org/data_pre.do?citid=901) from DFG-FOR816d
